...
text \<open>We define multigroupoids, extending the standard definition. We equip catoids with an operation
of inversion.\<close>
...
lemma (in st_multigroupoid) tgt_inv [simp]: 
  "\<tau> (inv x) = \<sigma> x"
  using local.Dst local.invr by fastforce

lemma (in st_multigroupoid) src_inv: 
  "\<sigma> (inv x) = \<tau> x"
  by simp

text \<open>The following lemma is from Theorem 5.2  of Jónsson and Tarski's BAO II article.\<close>
...
lemma (in st_multigroupoid) inv_s [simp]: 
  "inv (\<sigma> x) = \<sigma> x"
proof-
...
text \<open>As for catoids, we prove quantalic properties, lifting to powersets.\<close>

abbreviation (in st_multigroupoid) Inv :: "'a set \<Rightarrow> 'a set" where 
  "Inv \<equiv> image inv"

lemma (in st_multigroupoid) Inv_exp: 
  "Inv X = {inv x |x. x \<in> X}"
  by blast

lemma (in st_multigroupoid) Inv_un:
  "Inv (X \<union> Y) = Inv X \<union> Inv Y"
  by (simp add: image_Un)

lemma (in st_multigroupoid) Inv_Un:
  "Inv (\<Union>\<X>) = \<Union>{Inv X |X. X \<in> \<X>}"
  unfolding Inv_exp by auto

lemma (in st_multigroupoid) Invl: 
  "Src X \<subseteq> X \<star> Inv X"
  unfolding Inv_exp conv_exp using local.invl by fastforce

lemma (in st_multigroupoid) Invr: 
  "Tgt X \<subseteq> Inv X \<star> X"
  by (meson imageI image_subsetI local.invr local.stopp.conv_exp2)

lemma (in st_multigroupoid) Inv_strong_gelfand: 
  "X \<subseteq> X \<star> Inv X \<star> X"
proof-
  have "X = Src X \<star> X"
    by simp
...
text \<open>At powerset level, we can define domain and codomain operations explicitly as in 
relation algebras.\<close>
 
lemma (in st_multigroupoid) dom_def: 
  "Src X = sfix \<inter> (X \<star> Inv X)"
proof-
  {fix a
...
lemma (in st_multigroupoid) cod_def: 
  "Tgt X = sfix \<inter> (Inv X \<star> X)"
  by (metis local.st_mgpd.dom_def local.stfix_set local.stopp.conv_def multimagma.conv_def)

lemma (in st_multigroupoid) dom_def_var: 
  "Src X = sfix \<inter> (X \<star> UNIV)"
proof-
  {fix a
...
lemma (in st_multigroupoid) cod_def_var: 
  "Tgt X = sfix \<inter> (UNIV \<star> X)"

  by (metis local.ST_im local.sfix_im local.st_mgpd.dom_def_var local.stopp.conv_def local.tfix_im multimagma.conv_def)

lemma (in st_multigroupoid) dom_univ: 
  "X \<star> UNIV = Src X \<star> UNIV"
proof-
...
lemma (in st_multigroupoid) cod_univ: 
  "UNIV \<star> X = UNIV \<star> Tgt X"
  by (metis local.st_mgpd.dom_univ local.stopp.conv_def multimagma.conv_def)


subsection \<open>Groupoids\<close>

text \<open>Groupoids are simply functional multigroupoids.\<close>
...
text \<open>We provide a more direct axiomatisation.\<close>
...
lemma (in groupoid) rev1_eq: 
  "(y \<in> x \<odot> (inv z)) = (x \<in> y \<odot> z)"
  using local.lrgpd.rev2 by force

lemma (in groupoid) rev2_eq: 
  "(z \<in> (inv y) \<odot> x) = (x \<in> y \<odot> z)"
  by (simp add: local.lrgpd.rev1_eq)

text \<open>The following fact show that our axiomatisation captures indeed  groupoids.\<close>
...
text \<open>Hence st-groupoids are indeed single-set variants of categories in which all arrows
...
lemma (in groupoid) bao1 [simp]: 
  "x \<otimes> (inv x \<otimes> x) = x"
  by (simp add: local.pcomp_def)

lemma (in groupoid) bao2 [simp]: 
  "(x \<otimes> inv x) \<otimes> x = x"
...
lemma (in groupoid) bao6: 
  "Inv (x \<odot> y) = inv y \<odot> inv x"
  apply (rule antisym)
  using rev1_eq rev2_eq apply force
  by (clarsimp, metis imageI local.bao4 local.rev1_eq local.rev2_eq)


subsubsection \<open>Axioms of relation algebra\<close>

text \<open>We formalise an old theorem of Jónsson and Tarski, showing that groupoids lift to relation algebras
at powerset level. All axioms not related to converse have already been considered in earlier sections.\<close>

lemma (in groupoid) Inv_invol [simp]:
  "Inv (Inv X) = X"
proof-
...
lemma (in groupoid) Inv_contrav:
  "Inv (X \<star> Y) = Inv Y \<star> Inv X"
proof-
  have "Inv (X \<star> Y) = \<Union>{Inv (x \<odot> y) |x y. x \<in> X \<and> y \<in> Y}"
    unfolding conv_def image_def by blast
  also have "\<dots> = \<Union>{inv y \<odot> inv x |x y. x \<in> X \<and> y \<in> Y}"
...
text \<open>Hence we have shown that the powerset lifting of a groupoid is a relation algebra, 
an old result by Jónsson and Tarski in BAO II. We need to link this formally with relations
in an interpretation statement in the future.\<close>

text \<open>Jónsson and Tarski's axioms are different. It might be interesting to compare them formally with ours.
It might also be interested to use their partiality-by-closure approach in a setting with explicit
carrier sets.\<close>
...
lemma (in groupoid) Inv_inter:
  "Inv (X \<inter> Y) = Inv X \<inter> Inv Y"
  using local.Inv_compl by auto

lemma (in groupoid) Inv_Un:
  "Inv (\<Inter>\<X>) = \<Inter>{Inv X |X. X \<in> \<X>}"
proof-
  have "Inv (\<Inter>\<X>) = Inv (-(\<Union>{-X |X. X \<in> \<X>}))"
    by (simp add: Setcompr_eq_image)
  also have "\<dots> = - (Inv (\<Union>{-X |X. X \<in> \<X>}))"
    using local.Inv_compl by presburger
  also have "\<dots> = -(\<Union>{Inv (-X) |X. X \<in> \<X>})"
    by blast
  also have "\<dots> = -(\<Union>{-(Inv X) |X. X \<in> \<X>})"
    using local.Inv_compl by presburger
  also have "\<dots> = \<Inter>{Inv X |X. X \<in> \<X>}"
    by blast
  finally show "Inv (\<Inter>\<X>) = \<Inter>{Inv X |X. X \<in> \<X>}".
qed
