...
text \<open>Here I present  variants of 2-categories where the axioms are too strong. There is an Eckmann-Hilton
...
class two_st_multimagma_collapse = st_multimagma0 + st_multimagma1 +
  ...
  and s1_hom: "Src\<^sub>1 (x \<odot>\<^sub>0 y) = \<sigma>\<^sub>1 x \<odot>\<^sub>0 \<sigma>\<^sub>1 y" (* this is too strong *)
...
lemma comp_collapse: "x \<odot>\<^sub>0 y = x \<odot>\<^sub>1 y"
  by (smt (verit) local.interchange local.pm0.functionality_lem local.pm1.fun_in_sgl local.pm1.functionality_lem local.s0s1 local.src1_absorb local.ssmsg0.sts_msg.st_local local.stmsg1.sts_msg.st_local local.t0t1 local.tgt0_absorb local.tgt1_absorb multimagma.conv_atom stmsg0.tgt_comp_aux stmsg0.tgt_twisted_aux subset_singleton_iff)

lemma comp0_comm: "x \<odot>\<^sub>0 y = y \<odot>\<^sub>0 x"
  apply safe
  apply (smt (verit, best) comp_collapse empty_iff insert_eq_iff local.interchange local.pm0.fun_in_sgl local.s0t0 local.ssmsg0.sts_msg.st_local local.tgt0_absorb multimagma.conv_atom singleton_insert_inj_eq' stmsg0.src_twisted_aux stmsg0.tgt_twisted_aux)
  by (smt (verit, best) comp_collapse local.Dst0 local.interchange local.s0t0 local.stmsg0.ts_msg.src_comp_aux local.stmsg0.ts_msg.tgt_twisted_aux local.tgt0_absorb multimagma.conv_atom stmsg0.tgt_twisted_aux subsetD)
